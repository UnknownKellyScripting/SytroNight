...
function Kavo:DraggingEnabled(frame, parent)
    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- For mouse input, lock the mouse to prevent camera movement
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
                    -- Restore original mouse behavior when dragging ends
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
...
function Kavo:NewWin(Argstable)
    local kavName = Argstable.Name and Argstable["Name"] and kavName or "Kavo"
    ...
    Kavo.lastMainPosition = Main.Position
    ...
    close.MouseButton1Click:Connect(function()
	    ...
	    local tweenClose = game.TweenService:Create(Main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
	    ...
	    openButton.BackgroundColor3 = themeList.Header
	    openButton.Text = "Open"
	    openButton.TextColor3 = themeList.TextColor or Color3.fromRGB(245,245,245)
	    ...
	    local dragging = false
	    local dragInput, startPos, originalPos
	    ...
	    openButton.InputBegan:Connect(function(input)
	        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	            ...
	            originalPos = openButton.Position
	        end
	    end)
	    ...
	    openButton.InputChanged:Connect(function(input)
	        if input == dragInput and dragging then
	            ...
	            openButton.Position = UDim2.new(
	                0, originalPos.X.Offset + delta.X,
	                0, originalPos.Y.Offset + delta.Y
	            )
	        end
	    end)
	    ...
	    openButton.InputEnded:Connect(function(input)
	        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	            ...
	            if not hasDragged then
	                ...
	                Main.Visible = true
	                Main.Position = Kavo.lastMainPosition or Main.Position
	                Main.Size = UDim2.new(0, 0, 0, 0)
	                local tweenOpen = game.TweenService:Create(Main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    function Tabs:NewTab(tabName, icon)
		    ...
		    local function UpdateSize()
		        ...
		        game.TweenService:Create(page, TweenInfo.new(0.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {CanvasSize = UDim2.new(0, cS.X, 0, cS.Y)}):Play()
		    end
		    ...
		    if icon then
		        ...
		        tabIcon.Image = "rbxthumb://type=Asset&id=" .. icon .. "&w=150&h=150"
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewButton(Argstable)
                ...
                local tipINf = Argstable.InfoName and Arsgtable["InfoName"] and tipINf or "Information"
                local callback = Argstable.Function and Argstable["Function"] and callback or function() print("Button is clicked") end
            ...
            function Elements:NewTextBox(Argstable)
                ...
                local tTip = Argstable.InfoName and Argstable["InfoName"] and tTip or "Information"
                local callback = Argstable.Function and Argstable["Function"] and callback or function() print("Textbox is focused") end
            ...
            function Elements:NewToggle(Argstable)
			    ...
			    local nTip = Argstable.InfoName and Argstable["InfoName"] and nTip or "Information"
			    local defvalue = Argstable.Default and Argstable["Default"] and defvalue or false
			    local callback = Argstable.Function and Argstable["Function"] and callback or function() end
			
			    local toggled = defvalue
			    ...
			    btn.MouseButton1Click:Connect(function()
			        if not focusing then
			            ...
			            if rippleEffect and rippleEffect.Parent then
			                rippleEffect:Destroy()
			                rippleEffect = nil
			            end
			
			            toggleDisabled.ImageTransparency = toggled and 1 or 0
			            img.ImageTransparency = toggled and 0 or 1
			
			            currentTween = game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageTransparency = toggled and 0 or 1})
			            currentTween:Play()
			            pcall(callback, toggled)
			
			            rippleEffect = sample:Clone()
			            rippleEffect.Parent = btn
			            local x, y = ms.X - rippleEffect.AbsolutePosition.X, ms.Y - rippleEffect.AbsolutePosition.Y
			            rippleEffect.Position = UDim2.new(0, x, 0, y)
			            local len = 0.35
			            local size = (btn.AbsoluteSize.X >= btn.AbsoluteSize.Y) and (btn.AbsoluteSize.X * 1.5) or (btn.AbsoluteSize.Y * 1.5)
			            rippleEffect:TweenSizeAndPosition(UDim2.new(0, size, 0, size), UDim2.new(0.5, -size/2, 0.5, -size/2), 'Out', 'Quad', len, true, function()
			    ...
			    function TogFunction:UpdateToggle(newText, isTogOn)
			        ...
			        if isTogOn then
			            toggled = true
			            if currentTween then currentTween:Cancel() end
			            currentTween = game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageTransparency = 0})
			            currentTween:Play()
			            pcall(callback, toggled)
			        else
			            toggled = false
			            if currentTween then currentTween:Cancel() end
			            currentTween = game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageTransparency = 1})
			            currentTween:Play()
			            pcall(callback, toggled)
			        end
			    end
            ...
            function Elements:NewSlider(Argstable)
			    ...
			    local slidTip = Argstable.InfoName or "Information"
			    local maxvalue = Argstable.Max or 500
			    local minvalue = Argstable.Min or 10
			    local startVal = Argstable.Default or 50
			    ...
			    local fillWidth = math.clamp(((startVal - minvalue) / (maxvalue - minvalue)) * 149, 0, 149)
			    sliderDrag.Size = UDim2.new(0, fillWidth, 1, 0)
			
			    UICorner_3.Parent = sliderDrag
			
			    local sliderDot = Instance.new("ImageButton")
			    sliderDot.Name = "sliderDot"
			    sliderDot.Parent = sliderBtn
			    ...
			    val.Position = UDim2.new(0.35239, 0, 0.2727, 0)
			    ...
			    sliderBtn.MouseButton1Down:Connect(function()
			        if not focusing then
			            game.TweenService:Create(val, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {TextTransparency = 0}):Play()
			            local function updateSlider()
			                local mousePos = math.clamp(mouse.X - sliderBtn.AbsolutePosition.X, 0, 149)
			                sliderDrag:TweenSize(UDim2.new(0, mousePos, 1, 0), "InOut", "Linear", 0.05, true)
			                sliderDot.Position = UDim2.new(0, mousePos, 0.5, 0)
			                local Value = math.floor(((maxvalue - minvalue) / 149) * mousePos + minvalue)
            ...
            function Elements:NewDropdown(Argstable)
			    ...
			    local dropinf = Argstable.InfoName or "Information"
			    local defvalue = Argstable.Default or nil
			    ...
			    if defvalue and type(defvalue) == "string" then
			        ...
			        for _, opt in ipairs(list) do
			            if opt == defvalue then
			                found = true
			                break
			            end
			        end
			        if found then
			            itemTextbox.Text = defvalue
			            callback(defvalue)
			        end
			    end
			    ...
			    dropOpen.MouseButton1Click:Connect(function()
			        if not focusing then
			            if opened then
			                ...
			                c.Position = UDim2.new(0, x, 0, y)
			                local len = 0.35
			                local size = (dropOpen.AbsoluteSize.X >= dropOpen.AbsoluteSize.Y) and (dropOpen.AbsoluteSize.X * 1.5) or (dropOpen.AbsoluteSize.Y * 1.5)
			                c:TweenSizeAndPosition(UDim2.new(0, size, 0, size), UDim2.new(0.5, -size/2, 0.5, -size/2), 'Out', 'Quad', len, true, nil)
			                for i = 1, 10 do
			                    c.ImageTransparency = c.ImageTransparency + 0.05
			                    wait(len / 12)
			    ...
			    local moreInfoUICorner = Instance.new("UICorner")
			    moreInfoUICorner.CornerRadius = UDim.new(0, 4)
			    moreInfoUICorner.Parent = moreInfo
			    ...
			    for _, v in next, list do
			        ...
			        optionUICorner.Parent = optionSelect
			    ...
			    DropFunction.Refresh = function(newList)
			        newList = newList or {}
            ...
            function Elements:NewKeybind(Argstable)
                ...
                local keyinf = Argstable.InfoName and Argstable["InfoName"] and keyinf or "Information"
                local callback = Argstable.Function and Argstable["Function"] and callback or function()
                local first = Argstable.Default and Argstable["Default"] and first or Enum.KeyCode.E
            ...
            function Elements:NewColorPicker(colText, colInf, defcolor, callback)
                ...
                local colInf = Argstable.InfoName and Argstable["InfoName"] and colInf or "Information"
                local defcolor = Argstable.Default and Argstable["Default"] and defcolor or Color3.fromRGB(1,1,1)
                local callback = Argstable.Function and Argstable["Function"] and callback or function() print("Colorpicker callback generates the color rgb.") end
